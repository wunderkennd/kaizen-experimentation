...
//! - EQUIVALENCE / TOST (ADR-027): delta > 0; optional alpha in (0, 0.5);
//!   optional delta_relative > 0 and valid only for MEAN/RATIO.
...
mod tests {
    use super::*;
    use experimentation_proto::experimentation::common::v1::{
        BanditAlgorithm, Experiment, ExperimentType, EquivalenceTestConfig, MetaVariantObjective,
        MetricType, QuasiExperimentConfig, SwitchbackConfig, SyntheticControlMethod,
    };
    ...
    fn validate_starting_runs_equivalence_check() {
        ...
        let err = crate::validators::validate_starting(&exp).unwrap_err();
        assert!(err.message().contains("delta must be finite and > 0"));

        exp.equivalence_test = Some(equiv(0.05, None, 0.05));
        assert!(crate::validators::validate_starting(&exp).is_ok());
    }
}
