#!/usr/bin/env bash
# =============================================================================
# PGO Build: Profile-Guided Optimization for M2 Event Pipeline Service
# =============================================================================
...
#
# Usage:
#   ./scripts/pgo_build_pipeline.sh
#   PGO_PORT=50096 ./scripts/pgo_build_pipeline.sh
# =============================================================================
...
PGO_DIR="${PGO_DIR:-/tmp/pgo-data-pipeline}"
PGO_PORT="${PGO_PORT:-50096}"
PGO_METRICS_PORT="${PGO_METRICS_PORT:-9096}"
...
log()  { echo -e "${BLUE}[pgo-build-pipeline]${NC} $*"; }
...
if [[ -z "$LLVM_PROFDATA" ]]; then
    ...
    cd "$REPO_ROOT"
    cargo build --release --package experimentation-pipeline
    ok "Standard release build complete (no PGO): target/release/experimentation-pipeline"
    exit 0
fi
...
RUSTFLAGS="-Cprofile-generate=$PGO_DIR" \
    cargo build --release --package experimentation-pipeline 2>&1

INSTRUMENTED_BIN="$REPO_ROOT/target/release/experimentation-pipeline"
...
# Create temp buffer directory for profiling
TEMP_BUFFER_DIR=$(mktemp -d)

# Start the instrumented server
# KAFKA_BROKERS intentionally unreachable — events go to disk buffer,
# exercising the hot path without needing Docker Kafka.
PORT="$PGO_PORT" \
METRICS_PORT="$PGO_METRICS_PORT" \
...
# Wait for server to be ready (HTTP health check on metrics port)
log "Waiting for instrumented server (metrics port $PGO_METRICS_PORT)..."
for i in $(seq 1 30); do
    if curl -sf "http://localhost:${PGO_METRICS_PORT}/healthz" >/dev/null 2>&1; then
...
PGO_PORT="$PGO_PORT" bash "$SCRIPT_DIR/pgo_workload_pipeline.sh"
...
ok "PGO-optimized binary: target/release/experimentation-pipeline"

# Report sizes
BINARY_SIZE=$(du -h "$REPO_ROOT/target/release/experimentation-pipeline" | cut -f1)
log "Binary size: $BINARY_SIZE"

echo ""
echo "============================================================="
echo "  PGO BUILD COMPLETE: M2 Event Pipeline Service"
echo "============================================================="
echo "  Binary:   target/release/experimentation-pipeline"
echo "  Size:     $BINARY_SIZE"
echo "  Profiles: $PROFRAW_COUNT .profraw files merged"
echo "  Data:     $PGO_DIR/merged.profdata"
echo ""
echo "  Next steps:"
echo "    bash scripts/loadtest_pipeline.sh               # validate SLA"
echo "    TARGET_RPS=50000 bash scripts/loadtest_pipeline.sh  # stress test"
echo "============================================================="
