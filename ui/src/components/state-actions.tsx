...
export function StateActions({ state, onTransition }: StateActionsProps) {
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => allowed && !loading && setDialogOpen(true)}
        disabled={!allowed || loading}
        title={tooltip}
        className={`inline-flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 ${config.buttonClass} ${
          !allowed || loading ? 'opacity-50 cursor-not-allowed' : ''
        }`}
      >
        {loading && (
          <svg
