<!-- GENERATED from docs/agents/registry/infra-2.md by scripts/gen_agents.py — DO NOT EDIT.
     Edit the registry concept, then run `just gen-agents`. -->
# Infra-2: Data Stores & Project Scaffold

Owns the Pulumi project scaffold and persistent stores — RDS/Cloud SQL, Redis, object storage, secrets — on AWS and GCP.

- **Language**: Go (Pulumi)
- **Owned paths**: `infra/pkg/config/`, `infra/pkg/aws/database.go`, `infra/pkg/aws/cache.go`, `infra/pkg/aws/storage.go`, `infra/pkg/aws/secrets.go`, `infra/pkg/gcp/database.go`, `infra/pkg/gcp/cache.go`, `infra/pkg/gcp/storage.go`, `infra/pkg/gcp/secrets.go`, `infra/test/datastore_test.go`
- **Depends on**: infra-1
- **Work queue**: `gh issue list --label "infra-2" --state open` (claim protocol: `scripts/orchestration/README.md`)

Canonical identity & charter: [`docs/agents/registry/infra-2.md`](https://github.com/wunderkennd/kaizen-experimentation/blob/main/docs/agents/registry/infra-2.md) · Repo context anchor: [`CLAUDE.md`](https://github.com/wunderkennd/kaizen-experimentation/blob/main/CLAUDE.md)
