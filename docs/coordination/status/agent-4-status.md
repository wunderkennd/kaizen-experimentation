...
Sprint: 5.1
Focus: ADR-019 Portfolio Optimization (M4a stats layer)
Branch: work/bright-fox
...
- [x] **ADR-019 M4a stats layer — Portfolio Power Analysis** (2026-03-24, work/bright-fox, PR #261)
  - `crates/experimentation-stats/src/portfolio.rs` (new, ~650 LOC + 580 LOC tests)
    - `optimal_alpha(PortfolioParams)` — Storey-Tibshirani FDR optimal threshold; α* ∈ (0.001, 0.20)
    - `annualized_impact(AnnualizedImpactParams)` — 3-method projection (linear, compound, conservative); ≥ 0
    - `traffic_allocation_optimizer(TrafficAllocationInput)` — proportional allocation via n=2σ²(z+z)²/δ²; sum ≤ available
    - `portfolio_power_analysis(...)` → `PortfolioRecommendation` combined entry-point
    - 28 unit tests + 3 proptest invariants; all 249 experimentation-stats tests green
  - `proto/experimentation/analysis/v1/analysis_service.proto`:
    - `GetPortfolioPowerAnalysis` RPC + 5 new messages
  - `crates/experimentation-analysis/src/grpc.rs`:
    - `get_portfolio_power_analysis` handler; proto↔stats mapping; clean build
  - `crates/experimentation-stats/src/lib.rs`: `pub mod portfolio` registered
