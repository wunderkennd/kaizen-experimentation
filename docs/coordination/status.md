...
> **Last updated**: 2026-03-11 by Agent-2 (full pipeline E2E test + Criterion benchmarks, PR #124)
...
|-------|--------|--------|----------------|-------------------|------------|-------|
| Agent-1 | M1 Assignment | 🔵 Phase 4 In Progress | agent-1/feat/cumulative-holdout | Cumulative holdout priority assignment | — | M1.1–1.5 + M2.7 + M2.7b + M2.7c complete. Live bandit delegation done. Cold-start bandit done. PGO build pipeline merged (PR #116). k6 gRPC load test: p99 < 5ms at 10K rps (PR #122). Cumulative holdout priority assignment in PR #128 (open). 95 tests. |
...
| Agent-6 | M6 UI | 🔵 Phase 4 In Progress | agent-6/feat/phase4-next | Layer allocation chart | — | M1.25–1.27, M2.8–2.9, analysis tabs (PR #56), bandit dashboard (PR #60), live API integration. Phase 3 complete: surrogate/holdout/guardrail (PR #76), CATE lifecycle (PR #80), QoE/novelty/GST/Lorenz (PR #81). Phase 4: search/filter/sort (PR #90). RBAC UI: auth context, role-based button disabling. Performance targets merged (PR #108). Layer allocation bucket chart on experiment detail (PR #121). 239 tests pass. |
...
| 5 | Agent-3 ↔ Agent-4 (metric summaries → analysis) | 🟢 | ~50 contract tests (PR #127): 33 original column contracts + PERCENTILE, CUSTOM, guardrail, QoE-engagement, surrogate template coverage + StandardJob output schema validation across all 6 seed experiments. |
