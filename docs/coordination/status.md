...
| Agent-3 | M3 Metrics | 🔵 Phase 4 In Progress | agent-3/feat/custom-metric-type | CUSTOM metric type with SQL sandboxing | — | Phase 1–3 done. Kafka publisher (PR #64). M3↔M5 contracts (PR #68). Chaos tests (PR #69). Coverage improvements (PR #77). E2e pipeline tests (PR #79). Spark retry with exponential backoff (PR #86). Databricks notebook export (PR #87). CUSTOM metric type with SQL sandboxing (PR #91). |
| Agent-4 | M4a Analysis + M4b Bandit | 🔵 Phase 4 In Progress | agent-4/feat/analysis-service-scaffolding | M4a service scaffolding + GetInterferenceAnalysis | — | M1.14–1.19 merged. M2.1–2.6, M2.10 complete. M3.1–3.2 complete. M4.1 CATE in PR #70. M4b gRPC + Kafka consumer merged. M4a scaffolded: GetInterferenceAnalysis wired (Delta Lake → stats), 4 stub RPCs, 13 tests. |
