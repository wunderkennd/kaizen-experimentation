...
| L7 | Outcome record | Every run ends with `harness.outcome.v1` JSON (schema in `harness/schemas/outcome.v1.json`): issue, lane, agent label, model, result ∈ {pr_opened, branch_pushed, blocked, budget_exceeded, failed}, pr/branch refs, `total_cost_usd`, usage tokens, turns, wall-clock, session id, gate-hit list. Emitted via SDK `structuredOutput` + wrapper fields; posted as an issue comment (fenced `json` block under a `harness-outcome:` marker) and `$GITHUB_STEP_SUMMARY`. This is the #682 scorecard row and the H8 Phase 0 seed schema, machine-produced | claude session, 2026-07-11 |
