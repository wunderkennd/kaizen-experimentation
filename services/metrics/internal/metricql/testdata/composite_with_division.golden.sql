WITH operand_rows AS (
    ...
    WHERE experiment_id = 'exp_test'
      ...
      AND metric_id IN ('total_sessions', 'watch_time')
),
pivoted AS (
    SELECT
        user_id,
        variant_id,
        MAX(CASE WHEN metric_id = 'total_sessions' THEN metric_value END) AS m0,
        MAX(CASE WHEN metric_id = 'watch_time' THEN metric_value END) AS m1
    FROM operand_rows
...
SELECT
    ...
    user_id,
    variant_id,
    'm_composite_div' AS metric_id,
    ((m1 / NULLIF(m0, 0))) AS metric_value,
