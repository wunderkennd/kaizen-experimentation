...
class RemoteProvider(
    ...
    override suspend fun getAssignment(experimentId: String, attributes: UserAttributes): Assignment? {
        ...
        val body = JSONObject().apply {
            put("userId", attributes.userId)
            put("experimentId", experimentId)
            put("attributes", JSONObject(flattenProps(attributes.properties)))
        }

        val responseJson = post(url, body) ?: return null
        if (responseJson.optBoolean("isActive", false).not()) return null
    ...
    private fun flattenProps(props: Map<String, Any>): Map<String, String> =
